            ...
            <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2 lg:items-start">
                    <div class="rounded-xl border border-slate-200 bg-white p-4">
                        <div class="text-xs uppercase tracking-wider text-slate-500">Link firma</div>
                        @if ($absence->is_signed)
                        ...
                        @elseif ($absence->status === 'WAITING_SIGNATURE')
                            ...
                            @else
                                <div class="mt-3 text-sm text-slate-500">Non puoi generare il link firma per questa assenza.</div>
                            @endif
                            @if (session('signature_email'))
                                <div class="mt-4 rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-2 text-sm text-emerald-900">
                                    <div class="text-xs uppercase tracking-wider text-emerald-700">Email inviata</div>
                                    <div class="mt-2 font-medium">{{ session('signature_email') }}</div>
                                </div>
                            @endif
                            @if ($absence->signatureConfirmation && $absence->signatureConfirmation->expires_at)
                                <div class="mt-3 text-xs text-slate-500">
                                    Link attuale valido fino al {{ $absence->signatureConfirmation->expires_at->format('d.m.Y H:i') }}.
                                </div>
                            @endif
                        @else
                            <div class="mt-3 text-sm text-slate-500">La firma non e richiesta.</div>
                        @endif
                    </div>
                    <div class="rounded-xl border border-slate-200 bg-white p-4">
